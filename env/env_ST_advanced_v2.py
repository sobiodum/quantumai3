...
class StockTradingEnvAdvanced(gym.Env):
    ...
    def __init__(
        ...
        self.number_of_trade_budget = number_of_trade_budget
        ...
        if self.windows is not None:
            self.observation_space = spaces.Box(
                low=-np.inf, high=np.inf, shape=(self.calculate_state_shape(), self.windows)
            )
    ...
    def calculate_reward(self, end_total_asset, begin_total_asset):
        ...
        timefrequency_pentalty = 0
        if self.timestep_since_last_trade < 7:
            timefrequency_pentalty = - 0.005
        sharpe_ratio_reward = self.calculate_sharpe_ratio()
        if self.dynamic_weighting:
            # If Sharpe ratio is below 1, increase its importance
            if sharpe_ratio_reward < 1:
                self.sharpe_reward_weight += 0.10
                self.asset_reward_weight -= 0.10
            # If Sharpe ratio is above 1, decrease its importance
            else:
                self.sharpe_reward_weight -= 0.1
                self.asset_reward_weight += 0.1

        self.sharpe_reward_weight = np.clip(self.sharpe_reward_weight, 0, 1)
        self.asset_reward_weight = 1 - self.sharpe_reward_weight

        reward = self.asset_reward_weight * asset_reward + \
            self.sharpe_reward_weight * sharpe_ratio_reward + timefrequency_pentalty
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            for ticker in sell_tickers:
                ...
                # sell_stock now takes ticker and action_value
                actions[self.tickers_list.index(ticker)] = self._sell_stock(
                    ticker, action_value) * (-1)

            for ticker in buy_tickers:
                action_value = actions[self.tickers_list.index(ticker)]
                # buy_stock now takes ticker and action_value
    ...
    #! triggered errors by optuna as far as i can tell
        # end_date = datetime.strptime(
        #     self.episode_end_date[-1], '%Y-%m-%d').date()
        # start_date = datetime.strptime(
        #     self.episode_start_date[-1], '%Y-%m-%d').date()
        # num_days = (end_date - start_date).days
        # print("num_days", num_days)
        # equal_weighted_ann_return = (((self.calculate_portfolio_performance()) ** (
        #     365 / (self.day))) - 1) * 100
    # def calculate_portfolio_performance(self):
    #     # Fetch data for the start and end dates of the last episode
    #     start_data = self.df[self.df['date'] == "2000-03-30"]
    #     end_data = self.df[self.df['date'] == "2014-12-30"]

    #     # Extract starting and ending prices for all stocks
    #     start_prices = start_data.set_index('tic')['close'].to_dict()
    #     end_prices = end_data.set_index('tic')['close'].to_dict()

    #     # Calculate individual stock returns
    #     stock_returns = [(end_prices[tic] - start_prices[tic]) /
    #                      start_prices[tic] for tic in self.tickers_list]
