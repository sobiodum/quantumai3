...
class Portfolio_BBG_window_v2(gym.Env):
    ...
    def __init__(self,
        ...
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(
            self.lookback_window * (1 + self.stock_dim * 2 + len(self.indicator_list)),), dtype=np.float32)
    ...
    def calculate_reward(self, begin_total_asset, end_total_asset):
        ...
        # reward = annualized_return + drawdown_penalty + \
        #     frequent_flipping_pentalty + daily_return + trade_frequency_penalty
        reward = total_asset_reward * self.reward_scaling
        return float(reward)
    ...
    def _initiate_state(self):
        if self.initial:
            ...
            state = np.hstack(
                (cash_balance, stock_prices, stock_shares, indicators), dtype=np.float32)
    ...
    def _update_state(self):
        ...
        return np.hstack([cash_balance, stock_prices, stock_shares, indicators], dtype=np.float32)
    ...
    def _handle_terminal_condition(self):
        ...
        self.save_and_print_dfs(annualized_return)
        return self.state.flatten(), self.reward, self.terminal, False, {}
    ...
    def save_and_print_dfs(self, annualized_return):
        date_list = self.date_memory[:-1]
        asset_list = self.asset_memory[:-1]
        ...
        filename = f"bbg_env_runs_Ann_ret_{annualized_return}_{current_time}.csv"
